...
class KubeConfigMonitorTest {
    ...
    /** Subscribe to [KubeConfigMonitor.onClustersCollected] before start/refresh so emissions are counted. */
    private fun collectClusterEmissions(): MutableList<List<Cluster>> {
        val emissions = mutableListOf<List<Cluster>>()
        testScope.launch {
            kubeconfigMonitor.onClustersCollected { emissions.add(it) }
        }
        return emissions
    }
    ...
    fun `#start should initially parse and publish clusters`() = runTest(testDispatcher) {
        ...
        assertThat(emissions).containsExactly(listOf(cluster1))
    ...
    fun `#start coalesces multi-file addFile notifies into one full-list parse`() = runTest(testDispatcher) {
        ...
        assertThat(emissions).containsExactly(listOf(cluster1, cluster2))
    ...
    fun `#onFileChanged should reparse and publish updated clusters`() = runTest(testDispatcher) {
        ...
        assertThat(emissions).containsExactly(listOf(cluster1), listOf(cluster1Updated))
    ...
    fun `#updateMonitoredPaths should add and remove files based on KUBECONFIG env var`() = runTest(testDispatcher) {
        ...
        assertThat(emissions).containsExactly(listOf(cluster1), listOf(cluster2))

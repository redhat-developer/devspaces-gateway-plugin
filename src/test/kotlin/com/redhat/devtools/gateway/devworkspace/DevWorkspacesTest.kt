...
class DevWorkspacesTest {
    ...
    fun `#listWithResult resolves labels for Idea and non-Idea workspaces`() {
        ...
        assertThat(result.items.first { it.workspace.name == "idea-workspace" }.editor.kind)
            .isEqualTo(WorkspaceEditorKind.INTELLIJ_IDEA)
        assertThat(result.items.first { it.workspace.name == "idea-workspace" }.editor.tooltip)
            .isEqualTo("IntelliJ IDEA Ultimate (desktop)")
        assertThat(result.items.first { it.workspace.name == "code-workspace" }.editor.kind)
            .isEqualTo(WorkspaceEditorKind.VSCODE)
        assertThat(result.items.first { it.workspace.name == "code-workspace" }.editor.tooltip)
            .isEqualTo("VS Code - Open Source")
    }
    ...
    @Test
    fun `#listWithResult returns empty items on devworkspaces list 403`() {
        // given
    ...
    @Test
    fun `#listWithResult returns empty items on devworkspaces list 404`() {
        // given — plain 404 without CRD-missing body (namespace has no DevSpaces)
    ...
    @Test
    fun `#listWithResult returns empty items on devworkspaces list 404 CRD missing`() {
        // given — response body triggers isDevWorkspaceCrdMissing()
        mockListDevWorkspacesThrows(
            apiException(
                404,
                """{"message":"could not find the requested resource: devworkspaces","status":"Failure","details":{"kind":"devworkspaces"}}""",
            )
        )
    ...
    fun `#listWithResult throws ApiException on devworkspaces list 401`() {
        ...
        try {
            // given
            mockListDevWorkspacesThrows(ApiException(401, "Unauthorized"))

            // when/then
            assertThatThrownBy {
                devWorkspaces.listWithResult(namespace)
            }.isInstanceOf(ApiException::class.java)
    ...
    fun `#listWithResult resolves template-based JetBrains workspace`() {
        ...
        assertThat(result.items[0].editor.kind).isEqualTo(WorkspaceEditorKind.JETBRAINS)
        assertThat(result.items[0].editor.tooltip).isEqualTo("JetBrains")
    }

    @Test
    fun `#listWithResult includes templates in result`() {
        // given
        mockListDevWorkspaces(listOf(createDevWorkspaceItem("template-workspace", null)))
        mockListDevWorkspaceTemplates(
            listOf(
        ...
        assertThat(result.templates).containsKey("test-uid")
        assertThat(result.templates["test-uid"]).hasSize(1)
        assertThat(result.templatesUnavailable).isFalse()
    }

    @Test
    fun `#loadTemplates returns unavailable true for 401`() {
        // given
        mockListDevWorkspaceTemplatesThrows(ApiException(401, "Unauthorized"))

        // when
        val load = devWorkspaces.loadTemplates(namespace)
    ...
    @Test
    fun `#loadTemplates returns unavailable true for 403`() {
        // given
    ...
    @Test
    fun `#loadTemplates returns unavailable true for 404`() {
        // given
    ...
    @Test
    fun `#loadTemplates returns available with map on success`() {
        // given
        mockListDevWorkspaceTemplates(
            listOf(
                mapOf(
    ...
    private fun listWithResultTemplatesIgnored(exception: ApiException) {
        ...
        // then
        assertThat(result.items).hasSize(1)
        assertThat(result.items[0].editor.kind).isEqualTo(WorkspaceEditorKind.UNKNOWN)
        assertThat(result.items[0].editor.tooltip).isEqualTo("Unknown Editor")
    ...
    private fun mockListDevWorkspacesThrows(exception: ApiException) {
        every {
            anyConstructed<CustomObjectsApi>().listNamespacedCustomObject(
                "workspace.devfile.io",
                "v1alpha2",

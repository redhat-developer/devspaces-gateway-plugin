...
data class KubeConfigNamedCluster(
    ...
    companion object {
        ...
        fun fromKubeConfig(kubeConfig: io.kubernetes.client.util.KubeConfig): List<KubeConfigNamedCluster> {
...
data class KubeConfigNamedContext(
    ...
    companion object {
        fun fromMap(name: String, contextObject: Any?): KubeConfigNamedContext? {
        ...
        fun findContextForCluster(clusterName: String, kubeConfig: io.kubernetes.client.util.KubeConfig): KubeConfigNamedContext? {
            return (kubeConfig.contexts as? List<*>)?.firstNotNullOfOrNull { contextObject ->
                ...
                val contextEntry = fromMap(contextName, contextObject)
...
data class KubeConfigNamedUser(
    ...
    companion object {
        ...
        fun hasTokenAuth(kubeConfig: io.kubernetes.client.util.KubeConfig): Boolean {
            return (kubeConfig.users as? List<*>)?.any { userObject ->
                val userMap = userObject as? Map<*, *> ?: return@any false
                val userName = userMap["name"] as? String ?: return@any false
                fromMap(userName, userObject)?.user?.token != null
            } ?: false
        }
        
        fun findUserTokenForCluster(clusterName: String, kubeConfig: io.kubernetes.client.util.KubeConfig): String? {
            val contextEntry = KubeConfigNamedContext.findContextForCluster(clusterName, kubeConfig) ?: return null

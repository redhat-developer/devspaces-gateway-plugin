...
abstract class KubeConfigUpdate private constructor(
    ...
    class UpdateClientCert(
        ...
        override fun apply() {
            updateClientCert(context.context.user)
            updateCurrentContext(context.name)
        }

        private fun updateClientCert(username: String) {
            val config = KubeConfigUtils.getConfigByUser(context, allConfigs) ?: return
            ...
            config.users?.find { user ->
                ...
                Utils.removeValue(this, arrayOf("user", "token"))
            }
    ...
    class CreateContextWithClientCert(
        ...
        private fun createUser(allConfigs: List<KubeConfig>): KubeConfigNamedUser {
            val existingUserNames = getAllExistingNames(allConfigs) { it.users }
            val uniqueUserName =
                UniqueNameGenerator.generateUniqueName(clusterName, existingUserNames)
        ...
        private fun createCluster(allConfigs: List<KubeConfig>): KubeConfigNamedCluster {
            val existingClusterNames = getAllExistingNames(allConfigs) { it.clusters }
            val uniqueClusterName =
                UniqueNameGenerator.generateUniqueName(clusterName, existingClusterNames)
        ...
        private fun createContext(
            ...
            val uniqueContextName =
                UniqueNameGenerator.generateUniqueName(tempContext.name, existingContextNames)
